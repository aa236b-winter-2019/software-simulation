...
def encodestring(M,P,A,Time):
    #inputs
    # M,P,A - 3 unit arrays, float or int
    # Time - scalar, float or int
    #All values will be clipped to 5 decimal places
    ...
    for i in range(len3):
        ...
        if i != len3-1:   
            outputstring += "{:.5f}".format(A[i]) + ':'
          
        else:
            outputstring += "{:.5f}".format(A[i])
    
    #add time 
    outputstring+=';' + "{:.5f}".format(Time)
