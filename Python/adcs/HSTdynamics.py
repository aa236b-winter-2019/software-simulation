...
#   x_dot - linear equation containing ODEs for all states
    ...
    if init_state.size == 3: # angular velocity
        ...
        om_dot = -np.linalg.inv(J).dot(np.cross(om0, J.dot(om0) + rho))
        x_dot = om_dot
        ...
        # matrix linear equation
        rv_dot = np.block([[np.zeros((3,3)),      np.eye(3)],
                           [-(mu/r**3)*np.eye(3), np.zeros((3,3))]]).dot(init_state)
        x_dot = np.array(rv_dot)
    elif init_state.size == 7: # angular velocity and quaternion
        # unpack initial state
        om0 = init_state[:3]
        q0 = init_state[3:7]
        qhat = q0; # TODO: should be qmult(q0), not sure what that is

        # matrix linear equation
        om_dot = -np.linalg.inv(J).dot(np.cross(om0, J.dot(om0) + rho))
        q_dot = (1/2)*qhat*np.append(om0, 0)
        ...
        qhat = q0 # TODO: should be qmult(q0)
        r = np.linalg.norm(rvec)
        ...
        # matrix linear equation
        om_dot = np.linalg.inv(J).dot((Tau_g + Tau_D - np.cross(om0, J.dot(om0) + rho)))
        q_dot = (1/2)*qhat*np.append(om0, 0)

        rv_dot = np.block([[np.zeros((3,3)),      np.eye(3)],
                           [-(mu/r**3)*np.eye(3), np.zeros((3,3))]]).dot(np.append(rvec, vvec)) \
