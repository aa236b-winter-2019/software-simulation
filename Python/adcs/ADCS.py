...
h_equil=J**om_equil.T + rho*ones(1,np.length(om_equil))

om04 = np.array([2,2,10]).T
om04[2] = np.sqrt(np.aray(om_max,J33)**2 - (J11*om04(0))**2 - (J22*om04(1))**2)/J33
#opts-----------------------------
om4 = odeint(HSTdynamics, om04, tspan, args=(om,mu,J,rho,0))
h4 = np.multiply(J,om4.T) + rho*ones(1,np.length(om4))
...
# Plot Perturbed Dynamics
ax.plot3D(h4[1,:],h4[2,:],h4[3,:],color='b')
...
ax.plot3D( h_equil[1,:],h_equil[2,:],h_equil[3,:],'k*')
...
#------------------
#opts  = odeset('reltol', tol, 'abstol', tol)
tspan = np.arange(0,10,3*T)
#opts------------------
x = odeint(HSTdynamics, x0, tspan, args=(om,mu,J,rho,0))

# Plot Orbit
plt.figure()
ax=plt.axes(projection='3d')
plt.hold(True)
ax.plot3D(x[:,1],x[:,2],x[:,3],'r')
ax.axis('equal')
plt.xlabel('[km]')
plt.ylabel('[km]')
plt.zlabel('[km]')
...
om0 = np.aray( [0.25,0.25,2.5]).T # rad/s
x0 = np.array([om0,q0]).T

# ODE45 Solver for Attitude Dynamics
dt = 0.01
tspan = np.arange(0,dt,15)
tol = 1e-6
#-----------------------------
# opts  = odeset('reltol', tol, 'abstol', tol);
x = odeint(HSTdynamics, x0, tspan, args=(om,mu,J,rho,0))
...
ax1.plot(t,om_true[:,1])
plt.ylabel('\omega_1')
plt.xlabel('t')
ax1.set_title('HST Angular Velocity')
ax2.plot(t,om_true[:,2])
plt.ylabel('\omega_2')
plt.xlabel('t')
ax3.plot(t,om_true[:,3])
plt.ylabel('\omega_3')
plt.xlabel('t')
