...
% end
% figure;
% plot(t,vecnorm(h),'b','LineWidth',1),axis([0 6000 0 55])
% title('Angular Momentum'),xlabel('Time (s)'),ylabel('||h|| (kg m^2/s)')
% 
...
% alpha = pi/2000; dt = 0.2; tspan = 0:dt:pi/alpha;
...
%     %dx(:,ii) = [phi(:,ii) - phi_traj(:,ii); x(5:7,ii) - om_traj(:,ii); x(8:10,ii) - rho_traj(:,ii)];
%     %dx(:,ii) = [quat2phi(qmult(q_traj(:,ii))* x(1:4,ii)); x(5:7,ii) - om_traj(:,ii); x(8:10,ii) - rho_traj(:,ii)];
% end
% 
